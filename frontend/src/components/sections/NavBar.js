    ...
    if (this.props.loggedIn) {
      buttons = (
        <div
          ...
          className={(this.state.windowSize <= 600) ?
            'mobile-buttons-container is-hidden' : null
          }
        >
          <div>
            {/* Display the admin button or the home button */}
            {(localStorage.getItem('user_type') === 'admin') ?
                (this.props.displayAdmin) ?
                  <button
                    className='button'
                    onClick={() => this.updateAdmin(false)}
                  >
                    <img
                      className='icons'
                      src={HomeBlack}
          ...
          <div>
            ...
            >
              <img
                className='icons'
                src={LogoutBlack} alt='Logout Icon'
              />
              Logout
            </button>
          </div>
